...
describe('authService', () => {
  ...
  describe('sendPasswordResetLink', () => {
    it('should send password reset link successfully', async () => {
      ...
      expect(prisma.user.findUnique).toHaveBeenCalledWith({ where: { email } });
      expect(jwt.sign).toHaveBeenCalledWith({ userId: mockUser.id }, 'test_secret', { expiresIn: '15m' });
  ...
  describe('resetUserPassword', () => {
    it('should reset user password successfully', async () => {
      ...
      expect(prisma.user.findUnique).toHaveBeenCalledWith({ where: { id: decoded.userId } });
    ...
    it('should throw an error if token is invalid or expired', async () => {
      ...
      await expect(authService.resetUserPassword(token, newPassword)).rejects.toThrow('Invalid token');
      expect(jwt.verify).toHaveBeenCalledWith(token, 'test_secret');
    ...
    it('should throw an error if user not found after token verification', async () => {
      ...
      await expect(authService.resetUserPassword(token, newPassword)).rejects.toThrow('Invalid or expired token');
      expect(jwt.verify).toHaveBeenCalledWith(token, 'test_secret');
      expect(prisma.user.findUnique).toHaveBeenCalledWith({ where: { id: decoded.userId } });
      expect(bcrypt.hash).not.toHaveBeenCalled();
      expect(prisma.user.update).not.toHaveBeenCalled();
  ...
  describe('verifyUserEmail', () => {
    it('should verify user email successfully', async () => {
      ...
      const mockUser = { id: '1', email: 'test@example.com', emailVerificationToken: token, emailVerificationTokenExpiresAt: new Date(Date.now() + 3600000) };

      vi.spyOn(prisma.user, 'findUnique').mockResolvedValue(mockUser);
      vi.spyOn(prisma.user, 'update').mockResolvedValue({ ...mockUser, isVerified: true, emailVerificationToken: null, emailVerificationTokenExpiresAt: null });

      await authService.verifyUserEmail(token);

      expect(prisma.user.findUnique).toHaveBeenCalledWith({ where: { emailVerificationToken: token } });
      expect(prisma.user.update).toHaveBeenCalledWith({
        where: { id: mockUser.id },
        data: {
          isVerified: true,
    ...
    it('should throw an error if verification token is invalid or expired', async () => {
      ...
      await expect(authService.verifyUserEmail(token)).rejects.toThrow('Invalid or expired verification token');
      expect(prisma.user.findUnique).toHaveBeenCalledWith({ where: { emailVerificationToken: token } });
      expect(prisma.user.update).not.toHaveBeenCalled();
    });
    ...
    it('should throw an error if verification token has expired', async () => {
      ...
      await expect(authService.verifyUserEmail(token)).rejects.toThrow('Verification token has expired');
      expect(prisma.user.findUnique).toHaveBeenCalledWith({ where: { emailVerificationToken: token } });
      expect(prisma.user.update).not.toHaveBeenCalled();
    });
  });
